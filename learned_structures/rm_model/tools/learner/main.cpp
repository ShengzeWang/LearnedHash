...
void print_usage(std::ostream& out) {
  out << "Usage: rm_model_learner <input> [namespace] [models] [branching factor] [options]\n";
  out << "Options:\n";
  out << "  --param-grid <json>            Run grid configs from JSON\n";
  out << "  --optimize <json>              Write selector configs JSON\n";
  ...
  out << "  --disable-parallel-training    Disable parallel training for grid mode\n";
...
Options parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--no-code") {
      ...
      opts.param_grid = require_value(i, argc, argv, arg);
      ...
      opts.bounded = parse_size_arg(require_value(i, argc, argv, arg), arg);
      if (opts.bounded.value() == 0) {
        throw std::runtime_error("Bounded line size must be >= 1");
      }
    } else if (arg == "--max-size") {
      opts.max_size = parse_size_arg(require_value(i, argc, argv, arg), arg);
      ...
      opts.optimize = require_value(i, argc, argv, arg);
    } else {
