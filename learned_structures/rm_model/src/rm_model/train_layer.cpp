...
std::vector<std::unique_ptr<Model>> build_models_from(
  ...
  auto train_leaf = [&](std::size_t leaf_idx, std::vector<value_type> data_vec) {
    ...
    leaf_models[leaf_idx - first_model_idx] = detail::make_model(model_type, container);
  };

  auto fill_empty = [&](std::size_t from, std::size_t to) {
    for (std::size_t skip = from; skip < to; ++skip) {
      leaf_models[skip - first_model_idx] = detail::make_model(model_type, dummy_md);
    }
  };
...
TrainedModel train_two_layer(TrainingData<T>& data,
  ...
  detail::validate_model_spec<T>({layer1_model, layer2_model});
  ...
  auto top_model = detail::make_model(layer1_model, data);
  ...
  bool can_split = (split_idx > 0 && split_idx < data.len());
  if (can_split) {
    uint64_t key_at =
        top_model->predict_to_int(TrainingKeyOps<T>::to_model_input(data.get_key(split_idx)));
    uint64_t key_prev = top_model->predict_to_int(
        TrainingKeyOps<T>::to_model_input(data.get_key(split_idx - 1)));
    if (key_at <= key_prev) {
      // Degenerate split point; keep training robust by falling back to one-pass leaf training.
      can_split = false;
    }
  }

  std::vector<std::unique_ptr<Model>> leaf_models;
  auto build_all_leaf_models = [&]() {
    leaf_models = build_models_from(data, *top_model, layer2_model, 0, data.len(), 0,
                                    static_cast<std::size_t>(num_leaf_models));
  };

  if (!can_split) {
    build_all_leaf_models();
  } else {
    ...
    if (first_half == 0 || second_half == 0) {
      ...
      join(
          ...
          [&]() {
            hf2 = build_models_from(data, *top_model, layer2_model, split_idx, data.len(),
...
TrainedModel train_multi_layer(TrainingData<T>& data,
  ...
  for (const auto& model_type : model_list) {
    ...
    for (auto& partition : data_partitions) {
      ...
      auto model = detail::make_model(model_type, md_container);
  ...
  for (std::size_t midx = 0; midx < data_partitions.size(); ++midx) {
    ...
    auto model = detail::make_model(last_model, md_container);
