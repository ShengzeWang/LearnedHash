...
void TaskGroup::schedule(std::function<void()> task) {
  ...
  pool().enqueue([this, task = std::move(task)]() mutable {
    task();
    {
      ...
      --inflight_;
      --remaining_;
      // The waiter owns this TaskGroup. Notify before releasing the lock so the
      // stack-owned condition_variable cannot be destroyed during notify_all().
      cv_.notify_all();
    }
  });
}
...
void parallel_for_impl(std::size_t count,
  ...
  for (std::size_t task_idx = 0; task_idx < num_tasks; ++task_idx) {
    ...
    tp.enqueue([&, start, end]() {
      ...
      --remaining;
      if (remaining == 0) {
        // Keep notification inside the lock to protect the stack-owned
        // condition_variable from being destroyed by the waiting thread.
        done_cv.notify_one();
      }
    });
  }
